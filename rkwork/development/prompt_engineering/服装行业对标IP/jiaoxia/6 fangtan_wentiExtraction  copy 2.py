...
def chat_request(question_json):
    ...
    question = question_json['URL']
    # 在函数内部创建client实例
    client = OpenAI(
       base_url="http://aiagent-pre.chengwen.net/api/v1",  
       api_key="prechengwenai-rwcUdjvhFxYyxtUrI5CtYxouAj82stiiua3Sw8ueRO2L5J3xX0ZXm3rZ",
    )
    try:    #整个程序的try
        ...
        if json_match:
            ...
            try:
                ...
                matches = re.findall(r'\{.*?\}', data_chunk)
                print("matches", matches, type(matches[0]))
                result_data = json.loads(matches[0])
                question_json.update(result_data)

                print('整理好的json内容',repr(question_json), type(question_json))
                print("第二阶段正常")
                return question_json
            except Exception as e:
                
                print("第二阶段错误,错误内容如下",e,'\n',repr(matches[0]))
    except Exception as e:
        print("其他错误！",e)
...
if __name__ =='__main__':
    ...
    task =[]
    # 构造输入数据
    with open('VFU运动旗舰店.json','r', encoding='utf-8') as f:
    ...
    with multiprocessing.Pool(processes=30) as pool:
        results = pool.map_async(chat_request, samples_data)     
            ...
            if mod == 0:
                with open('temp.txt', 'w', encoding='utf-8') as f:
                    for j in data:
                        f.write(str(j) +'\n')
                        f.flush()
                print('备份了',int(index)/20,'次')  
        print("执行完毕！")
        with open('VFU运动旗舰店".txt', 'w', encoding='utf-8') as f:
            for j in data:
                f.write(str(j) +'\n')
                f.flush()
...
with open('VFU运动旗舰店".json', 'a+', encoding='utf-8') as f:
