...
def chat_request(question_json):
    ...
    question = question_json['视频文案']
    # 在函数内部创建client实例
    client = OpenAI(
       base_url="http://aiagent-pre.chengwen.net/api/v1",  
       api_key="prechengwenai-dPC3ZGs3tH7QRGEG9oAryZiGvs5wtDsPUQKoO0cMP6yvRF3mGBn9aTwjLSOd21"
    )
    ...
    try:    #整个程序的try
        ...
        if json_match:
            ...
            try:
                ...
                print('question_result',question_json)
...
with multiprocessing.Pool(processes=20) as pool:
    ...
    results = pool.map_async(chat_request, task)      # 第二个参数一定是 列表元素，传进去会传其中的元素
...
with open('服装行业访谈型金句提取.json', 'w', encoding='utf-8') as f:
