...
def chat_request(question_json):
    question = question_json['脚本内容']
    # 在函数内部创建client实例
    client = OpenAI(
       base_url="http://aiagent-pre.chengwen.net/api/v1",  
       api_key="prechengwenai-gVx9ABVAZnCiIRYPRZVmC8NeGAFQsvmNSXb66vEErzo8NdUrdOyqlXfeoI7rz"
    )
    try:    #整个程序的try
        ...
        if json_match:
            ...
            try:        # openai数据格式返回的try
                ...
                # 去除无关字段
                question_json.pop('脚本内容')
                question_json.update(result_data)
                print('openai输出位置',type(question_json))
            ...
            try:
                ...
                question_json.pop['脚本内容']
                question_json.update(result_data)
                print('doubao128k输出位置',type(question_json))
                return question_json
            except Exception as e:
                print("douboa出错了！",e)
    except Exception as e:
...
if __name__ =='__main__':
    ...
    task =[]
    # 构造输入数据
    with open('访谈型视频文案脚本复现助手.json','r', encoding='utf-8') as f:
        data = json.loads(f.read())
        for i in data:      
            task.append({'视频ID':i['视频ID'],'脚本内容':i['访谈型视频文案脚本复现助手']})
    ...
    with multiprocessing.Pool(processes=20) as pool:
        ...
        for index,result in enumerate(results.get()):
            result_list.append(result)              # 这里不应该有文案内容，因该去除视频文案

    end = time.time()
    print(f"最终执行时间{end - start}")

    with open('服装行业访谈型问题提取.json', 'w', encoding='utf-8') as f:
        json.dump(result_list, f, ensure_ascii=False, indent=4)


'```json\n{\n  "访谈类文案问题提取助手": {\n    "问题一": "第一个问题很好奇，你为什么会接受我的探访？理论上，你低调是最好的吗？",\n    "问题二": "就很多人其实都还蛮介意的，为什么你这么愿意就是去分享了？你是很想，就是自己能尽一份力量，能够对这个行业成长一些吗？",\n    "问题三": "你觉得具备什么样条件的时候你就可以开放加盟呢？",\n    "问题四": "我感觉你身上给自己背负了好多责任哦。",\n    "问题五": "那你自己的选品标准是怎么样的呢？",\n    "问题六": "别人就看到你两木里表面的枝繁叶茂，但他看不到背后的更是庞大。嗯，我看到你们今年业绩还翻倍了是吧？",\n    "问题七": "你真的对自己要求好高哦，那你觉得从一个小小的工作室到今天，最重要是做对了哪几件事情？",\n    "问题八": "我觉得现在很多人有一个误区，他觉得好像是因为店大才生意好，其实我们是一步一步成长为一个大店的，而不是上来就是一个大店，所以就一心都想着怎么样给客户提供好的产品和服务。",\n    "问题九": "我觉得这个很重要啊，很多人他其实不知道如何给客户提供好的产品和服务。",\n    "问题十": "那你平时会看什么东西吗？",\n    "问题十一": "所以你更多的，真的就是来自于无意之中的用户反馈。比如说什么好卖，客户跟你说什么好看，那你会下一次重复的用这种搭配，然后又让他不一样，又加一些东西嘛，是类似这样子来磨练自己嘛？",\n    "问题十二": "哎，我其实跟你聊着聊着，我还蛮开心的，就是可能真的没有那么复杂，就始终站在用户的角度，不停的在磨练自己的产品、自己的搭配。",\n    "问题十三": "会拍照到群里或者是干嘛吗？",\n    "问题十四": "其实我内心蛮感动的，因为我很希望给大家传递一些没有捷径的这种价值理念。就是长期你的热爱，加上你的脚踏实地的这种经营，练基本功，拿到成绩，我觉得就是一个非常正向的案例。",\n    "问题十五": "那么，为什么会选择做服装呢？"\n  }\n}\n```'