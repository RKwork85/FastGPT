...
with open(file_name, 'r', encoding='utf-8') as file:
    for line in file:
        ...
        # 检查去除空白字符后的行是否包含“参考示例”或“提示词”

        if str1 in stripped_line :
            start = True
            list_start.append(start)      

            number += 1

            if number == len(list_start):
                list_data.append(result)
                print(result)
                result = ''

            continue
        elif str2 in stripped_line:
            continue 
        else:
...
with open ("question_prompt.txt", 'w', encoding='utf-8') as f:
